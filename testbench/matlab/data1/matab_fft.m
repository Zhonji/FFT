...
fp = fopen('data_before_fft_213.txt','w');

for i = 1:length
   if(data_before_fft(i)>=0)
       temp= dec2bin(data_before_fft(i),16);
   else
       temp= dec2bin(data_before_fft(i)+2^16,16);
   end
    for j=1:16
        fprintf(fp,'%s',temp(j));
    end
...
for i = 1:length
   if(data_real(i)>=0)
       temp_real= dec2bin((data_real(i)/128),16);
   else
       temp_real= dec2bin((data_real(i)/128)+2^16,16);
   end
    for j=1:16
...
for i = 1:length
   if(data_imag(i)>=0)
       temp_imag= dec2bin(data_imag(i)/128,16);
   else
       temp_imag= dec2bin(data_imag(i)/128+2^16, 16);
   end
    for j=1:16
